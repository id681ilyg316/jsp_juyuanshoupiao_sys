...
public class AdminDaoImpl implements AdminDao{
	...
	private static final String UPDATE_FLIGHT_SQL="update T_FLIGHT set F_A_NO=?,F_START=?,F_DIST=?,F_PRICE=?,F_TIME=?,name=? where F_NO = ?";
	...
	public void deleteAirPlane(AirPlane airPlane) throws FMException{
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			throw new FMException("外键约束！删除演出团队失败！");
		}
	}
	...
	public void insertFlight(Flight flight) throws FMException{
		QueryRunner runner= new QueryRunner(DaoUtils.getSource());
		try {
			runner.update(INSERT_FLIGHT_SQL, flight.getAirPlane().getNo(),flight.getStart(),flight.getDist(),flight.getPrice(),flight.getTime(),flight.getName());
			System.out.println(flight);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			throw new FMException("添加演出失败！ 没有该演出团队！");
		}
	...
	public void updateFlight(Flight flight) throws FMException{
		QueryRunner runner =new QueryRunner(DaoUtils.getSource());
		try {
			runner.update(UPDATE_FLIGHT_SQL,flight.getAirPlane().getNo(),flight.getStart(),flight.getDist(),flight.getPrice(),flight.getTime(),flight.getName(),flight.getNo());
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			throw new FMException("修改演出信息失败！");
		}
	}
	...
	public Statistic makeStatistic(String start) {
		...
		try {
			statistic.setWeekFlight(runner.query(QUERY_FLIGHT_NUM_WEEK_SQL, new CountResultSetHandler()));
			statistic.setMonthFlight(runner.query(QUERY_FLIGHT_NUM_MONTH_SQL, new CountResultSetHandler()));
			statistic.setTotalFlight(runner.query("select count(*) from T_FLIGHT where F_START ='"+start+"'", new CountResultSetHandler()));
			
			statistic.setWeekOrder(runner.query(QUERY_ORDER_NUM_WEEK_SQL, new CountResultSetHandler()));
			statistic.setMonthOrder(runner.query(QUERY_ORDER_NUM_MONTH_SQL, new CountResultSetHandler()));
			statistic.setTotalOrder(runner.query("select count(*) from T_ORDER WHERE O_F_NO IN (SELECT F_NO FROM t_flight WHERE F_START='"+start+"');", new CountResultSetHandler()));
			
			statistic.setWeekIncome(runner.query(QUERY_INCOME_WEEK_SQL, new FloatResultSetHandler()));
			statistic.setMonthIncome(runner.query(QUERY_INCOME_MONTH_SQL, new FloatResultSetHandler()));
			statistic.setTotalIncome(runner.query("select sum(F_PRICE) from T_ORDER o,T_FLIGHT f where o.O_F_NO=f.F_NO and O_IS_PAYED=1 and O_IS_CANCELED=0 AND f.F_START='"+start+"'", new FloatResultSetHandler()));
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
